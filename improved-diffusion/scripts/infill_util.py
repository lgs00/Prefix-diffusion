...
def langevin_fn4(debug_lst, model_control, model3, label_ids, step_size, sample, mean, sigma,
                 alpha, t, prev_sample,image_feature): # current best.
    if t[0].item() < 10:
        K = 0
    else:
        K = 3
    ...
    if t[0].item() >0:
        ...
        tt = 200
    img_label =th.ones( (sample.size(0)))
    img_label = img_label.cuda()
    input_embs_param = th.nn.Parameter(sample)
    image_feature = image_feature.unsqueeze(0).repeat(sample.size(0),1,1)
    image_feature_param = th.nn.Parameter(image_feature)
    print("bi",input_embs_param.data)
    if False:
        input_embs = th.cat([input_embs_param, tgt_embs], dim=1)
        debug_lst.append(get_score(input_embs, label_ids2, model_control, t=tt))
    ...
    with th.enable_grad():
        for i in range(K):
            optimizer = th.optim.Adagrad([input_embs_param], lr=0.1)
            #pdb.set_trace()
            #print("b",input_embs_param.grad)
            optimizer.zero_grad()
            # print(input_embs_param.shape, label_ids.shape)
            # input_embs_param.shape=64*64*16,label_ids.shape=64*64(词性id，64组一样的id，长度为64）
            # model_out含有logits和loss，logits=64*64*21
            model_out = model_control(input_embs=input_embs_param, t=tt, img = image_feature,img_label=img_label)

            #coef = 0.1 # prev default.
            coef =0.0001
            # coef = 0.0005

            # coef=1.   # sigma.shape=64*64*16,全是一样的值,mean.shape=64*64*16,mean是由训练的model得到后求来，
            if sigma.mean() == 0:   # false
                logp_term = coef * ((mean - input_embs_param) ** 2 / 1.).mean(dim=0).sum()
            else:
                logp_term = coef * ((mean - input_embs_param)**2 / sigma).mean(dim=0).sum()
            print(model_out.loss, f'start_{i}', logp_term.item(),
                  t[0].item(), sigma.mean().item())
            model_out.loss = (10**10)*model_out.loss
            model_out.loss = (10**10)*model_out.loss
            loss = model_out.loss  + logp_term

            loss.backward()
            #th.nn.utils.clip_grad_norm_(parameters=[input_embs_param,image_feature_param], max_norm=10, norm_type=2)
            optimizer.step()
            print("l",input_embs_param.grad)
            epsilon = th.randn_like(input_embs_param.data)  # epsilon.shape=64*64*16
            input_embs_param = th.nn.Parameter((input_embs_param.data + 0.0*sigma.mean().item() * epsilon).detach())
            #image_feature_param = th.nn.Parameter((image_feature_param.data).detach())
            #input_embs_param = th.nn.Parameter((input_embs_param.data +
            #                                   np.sqrt(2*sigma.mean().item()) * epsilon).detach())
    model_out = model_control(input_embs=input_embs_param, t=tt, img = image_feature,img_label=img_label)
    print("li",input_embs_param.data)
    print(model_out.loss, 'end')
    return input_embs_param.data
