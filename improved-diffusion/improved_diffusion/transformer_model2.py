...
class TransformerNetModel2(nn.Module):
    ...
    def __init__(
        ...
        self.img_project = MLP((768,768*2,768*5))
        ...
        self.output_down_proj = nn.Sequential(nn.Linear(config.hidden_size, config.hidden_size),
                                              nn.Tanh(),nn.Dropout(config.hidden_dropout_prob), nn.Linear(config.hidden_size, out_channels))
    ...
    def forward(self, x, timesteps, img=None, y=None, src_ids=None, src_mask=None):
        """
        Apply the model to an input batch.
        ...
        img_inputs = self.img_project(img.float()).view(-1, 5, 768)
