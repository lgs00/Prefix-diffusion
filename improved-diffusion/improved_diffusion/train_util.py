...
class TrainLoop:
    ...
    def run_loop(self):
        while (
            ...
            or self.step + self.resume_step < self.lr_anneal_steps # lr_anneal_steps=200000, resume_step=0
        ):
            #print("175:",self.lr_anneal_steps)
    ...
    def forward_backward(self, batch, cond):
        zero_grad(self.model_params)    # 模型参数
        # batch.shape[0]=64,batch=64*64*16,self.microbatch=64
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(dist_util.dev())
            micro_cond = {
                k: v[i : i + self.microbatch].to(dist_util.dev())
                for k, v in cond.items()
            }   # micro_cond含有input_ids(shape=64*64),img(64*1*512)
            img = micro_cond['img']
            last_batch = (i + self.microbatch) >= batch.shape[0]
            # t 是batch中每个样本的时间步，64个(0-2000)的随机数,weights是每个时间步的权重
            t, weights = self.schedule_sampler.sample(micro.shape[0], dist_util.dev())
            # print(micro_cond.keys())
            compute_losses = functools.partial(
                self.diffusion.training_losses,
                self.ddp_model,
                micro,  # micro.shape=64*64*16
                t,
                img,
                model_kwargs=micro_cond,
            )

            if last_batch or not self.use_ddp:  # use_ddp=true,last_batch=true
                losses = compute_losses()
            else:
                with self.ddp_model.no_sync():
                    losses = compute_losses()
