...
def helper_tokenize_encode(sentence_lst, vocab_dict, model, seqlen, data_args, padding_mode, img_lst, label_lst=None):
    result_train_lst = []
    group_lst = defaultdict(list)
    with torch.no_grad():
    ...
    if data_args.experiment_mode == 'lm':
        if data_args.modality == 'e2e-tgt':
            ...
            if split == 'train':
                ...
                path = f'{data_args.e2e_train}/coco_train_text_img.pkl'
            elif split == 'valid':
                print('loading form the VALID set')
                path = f'{data_args.e2e_train}/coco_val_text_img.pkl'
            elif split == 'test':
                print('loading form the TEST set')
                path = f'{data_args.e2e_train}/coco_test_text_img.pkl'
